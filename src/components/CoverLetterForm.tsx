...
const inputCls =
  'w-full px-3 py-2 rounded-lg border border-slate-200 text-slate-900 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors';
...
export function CoverLetterForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-6 text-left"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-slate-900">Generate cover letter</h3>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label htmlFor="cl-role" className="block text-sm font-medium text-slate-700 mb-1">
          ...
          <div>
            <label htmlFor="cl-hm" className="block text-sm font-medium text-slate-700 mb-1">Hiring manager (optional)</label>
            <input id="cl-hm" className={inputCls} value={hiringManager} onChange={(e) => setHiringManager(e.target.value)} placeholder="Ms. Jane Doe" />
          </div>
          <div>
            <label htmlFor="cl-notes" className="block text-sm font-medium text-slate-700 mb-1">Extra paragraph (optional)</label>
        ...
        {!valid && (
          <p className="text-xs text-slate-400 mt-3">Enter company and role to enable download.</p>
        )}
        ...
        <div className="flex gap-3 mt-5">
          <DownloadButton
            ...
            disabled={!valid}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 disabled:opacity-50 text-white rounded-lg transition-colors"
          />
          <DownloadButton
            label="Word"
            ...
            disabled={!valid}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-lg transition-colors"
          />
        </div>
      </div>
    </div>
